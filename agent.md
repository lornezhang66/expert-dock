...
- Helper 复制文件后，调用本机 WorkBuddy 自带的 `register_expert.py` 完成注册；找不到或执行失败时回滚本次写入并提示用户。
...
8. Helper 定位并调用 WorkBuddy 自带的 `register_expert.py` 注册专家；成功后提示重启或刷新 WorkBuddy。
...
- 注册脚本必须来自检测到的 WorkBuddy 安装目录，绝不执行专家包内的可执行文件；运行参数不得经过 Shell 字符串拼接。
...
- Helper 优先使用单一跨平台代码库，但不要为了复用引入 Electron；只需注册 `expertdock://`、扫描并校验目录、下载、校验、解压、备份、原子写入和启动注册脚本。
...
- 安装成功后，专家文件位于当前用户的 WorkBuddy 私域专家目录，并已通过 WorkBuddy 自带注册脚本注册。
...
1. 在目标 Windows/macOS 环境验证 WorkBuddy 用户目录、专家目录、可靠识别特征和 `register_expert.py` 的实际位置与调用方式，并据此固定扫描规则。
2. 完成数据表、登录和资源所有权校验。
